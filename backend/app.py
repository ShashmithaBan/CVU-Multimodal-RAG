...
def extract_code_from_video(video_id):
    ...
    try:
        ...
        # Build formatted_extraction string for frontend rendering
        frames_analyzed = sum(b.get('frames_used', 0) for b in all_code_blocks)
        formatted_extraction = '\n\n'.join(
            f"### {b['filename']}\n```{b.get('language', '')}\n{b['code']}\n```"
            for b in all_code_blocks
        )

        # Build summary / usage hint
        file_list = ', '.join(f"`{b['filename']}`" for b in all_code_blocks)
        summary = (
            f"Extracted from **{len(all_code_blocks)} file(s)**: {file_list}.\n"
            f"Confidence: {'HIGH ✅' if len(valid_blocks) == len(all_code_blocks) else 'MEDIUM ⚠️'}. "
            f"Copy any snippet using the button above each block."
        )
